...
class SwarmInterface(Node):
    ...
    def agent_body_velocity_callback(self, agent_body_velocity):
        ...
        # print('this is the velocity that i have to follow : ', convert_body_vel_to_spatial_vel(agent_body_velocity[0], agent_body_velocity[1], agent_body_velocity[2], self.vehicle_odometry.q), '\n')
        # sim_quaternions = [self.vehicle_sim_pose.pose.orientation.w, self.vehicle_sim_pose.pose.orientation.x, self.vehicle_sim_pose.pose.orientation.y, self.vehicle_sim_pose.pose.orientation.z]
        ...
        # print('this is my velocity', self.agent_spatial_velocity, '\n\n')
    ...
    def vehicle_position_callback(self, vehicle_position):
        ...
        if vehicle_position_info['vehicle_id'] != self.vehicle_id:
            ...
            # sim_to_qgc(self.vehicle_sim_pose.pose.position.x, self.vehicle_sim_pose.pose.position.y, self.vehicle_sim_pose.pose.position.z)
            # sim_to_qgc(neighbour_vehicle_position[0], neighbour_vehicle_position[1], neighbour_vehicle_position[2])
            ...
            # print('relative orientation', neighbour_relative_orientation, '\n\n')
            ...
            # relative_body_position = [relative_body_position[0], relative_body_position[1], relative_body_position[2]]
            # print('this is relative pose in qgc (body) : ', relative_body_position)
            # back = convert_body_vel_to_spatial_vel(relative_body_position[0], relative_body_position[1], relative_body_position[2], self.vehicle_odometry.q)
            # print('this is vel in spatial : ', back)
        ...
        if len(self.all_neighbours_relative_position_in_body) > 0:
            ...
            # print('publishing data to guidance algorithm')
    ...
    #         neighbour_vehicle_relative_position = [(neighbour_vehicle_position[0] - self.vehicle_sim_pose.pose.position.x),
    #                                                neighbour_vehicle_position[1] - self.vehicle_sim_pose.pose.position.y,
    #                                                (neighbour_vehicle_position[2] - self.vehicle_sim_pose.pose.position.z)]
            
    #         # rotate relative pose to body frame 
    #         sim_quaternions = [self.vehicle_sim_pose.pose.orientation.w, self.vehicle_sim_pose.pose.orientation.x, self.vehicle_sim_pose.pose.orientation.y, self.vehicle_sim_pose.pose.orientation.z]
    #         relative_body_position = get_relative_position_in_body(neighbour_vehicle_relative_position, sim_quaternions)
    #         # relative_body_position = neighbour_vehicle_relative_position
    #         # print('this is relative pose in sim : ', relative_body_position)
    #         relative_body_position = [relative_body_position[0], relative_body_position[1], relative_body_position[2]]
    #         print('this is relative pose in qgc : ', relative_body_position)
    ...
    #         # print(vehicle_position_info['vehicle_id'])
    #         # update neighbour or add a new neighbour (does not take into account for checking the old neighbour data and removing them from the array)
    #         if self.neighbours_last_seen.get(vehicle_position_info['vehicle_id'], False):
    #             # update
    #             index = self.neighbours_last_seen[vehicle_position_info['vehicle_id']]['index']
    #             self.all_neighbours_relative_position_in_body[index] = relative_pose_stamped
    #             # print('updating the array', index)
    #         else:
    #             # add 
    #             # print('adding into the array')
    #             self.neighbours_last_seen[vehicle_position_info['vehicle_id']] = {'last_time_stamp': time.time(),
    #                                                                          'index': len(self.all_neighbours_relative_position_in_body)}
    #             self.all_neighbours_relative_position_in_body.append(relative_pose_stamped)
    ...
    def publish_position_setpoint(self, x: float, y: float, z: float, is_leader_follower: bool):
        ...
        if self.leader_id == self.vehicle_id and self.done_with_takeoff:
            ...
            # print('altitude error : ', altitude_error, 'corresponding adjustment : ', z)
        ...
        msg.yaw = float('nan')
        # if is_leader_follower:
        #     msg.yaw = np.arctan2(y, x)
        # print('this is my yaw : ', msg.yaw)
        msg.timestamp = int(self.get_clock().now().nanoseconds / 1000)
